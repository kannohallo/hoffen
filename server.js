...
mongoose.connect("mongodb+srv://kannohallo:<Innovando.2002>@hoffendatabase.hqous.mongodb.net/<dbname>?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }, () => {
    console.log("MongoDB ✔")
})
client.on("ready", () => {
...
if (msg.content.startsWith(prefix+"ban")) {   //Hacemos el comando
const args = msg.content.slice(prefix.length).trim().split(/ +/g);	if (!msg.guild.me.permissions.has('BAN_MEMBERS')) {  //Empezamos definiendo los permissions del bot
...
if (!msg.member.permissions.has('BAN_MEMBERS')) { //Ahora definimos las permissions del usuario 
...
if (msg.content.startsWith (prefix + "mute")) {
    let user = message.mentions.users.first() //Establecemos la mención al usuario
    if (!user) return message.channel.send("No user") //Si no se menciona a un usuario retorna
    let data = MuteDB.findOne({ userID: user.id }) //"Obtenemos" el objeto si hay algo guardado 
    if (data) return message.channel.send("Usuario ya está Muteado") //Si lo hay quiere decir que el usuario está muteado, por lo tanto el mensaje.
    ...
    } else { //de lo contrario lo crearemos
    ...
    message.guild.channels.cache.forEach(async (channel) => { //Esto obtendrá todos los canales del servidor y modificará los permisos del rol, denegando que envíe mensajes
